...
class Updater {
    ...
    */
    ...
    function __construct( modX &$modx ) {
        ...
        $this->debug = $this->modx->getOption('updater.debug', null, false);
     ...
     */
    private function is_release_level($v, $level = 'pl') {
     ...
     */
    private function is_version_level($v, $level = 'patch') {
        ...
        switch ($level) {
            case 'release':
                if ($vMajor==$cMajor && $vMinor==$cMinor && $vPatch==$cPatch && $vRelease>=$cRelease) {
                    $result = true;
                }
                break;
            case 'patch':
                ...
                if ($vMajor==$cMajor && $vMinor==$cMinor && $vPatch>=$cPatch) { $result = true; }
                break;
            case 'minor':
                if ($vMajor==$cMajor && $vMinor>$cMinor) { $result = true; }
                break;
            case 'major':
                if  ($vMajor>$cMajor) {$result = true;}
                break;
            default:
                $result = false;
        }
    ...
    private function is_patch_build($v) {
    ...
    public function isPackageRefreshNeeded() {
        return $this->getPackageData();
    }

    public function refreshPackageData()
    {
     ...
     */
    public function refreshCoreDownload( $type='traditional' ) {
        $time = microtime(true);
    ...
    public function isCoreRefreshNeeded() {
     ...
     * @param array $cd
     */
    private function setCoreVersion(array $cd) {
        $this->coreData = $cd;
     ...
     */
    private function filterCoreVersions(array $versions = null)
    {
        /*
     ...
     */
    private function getUpdateableCoreVersions(array $versions, $compare = '') {
        if ($compare == "") {
     ...
     */
    public function refreshCoreVersion() {
     ...
     */
    public function getUpdateCandidates() {
     ...
     */
    public function generateWidget( $area = "" ) {
        ...
        /*
        $output .= "<script type='text/javascript' src='".MODX_ASSETS_URL.'components/updater/js/spin.min.js'."'></script>";
        $output .= "<script type='text/javascript' src='".MODX_ASSETS_URL.'components/updater/js/updater.widget.js'."'></script>";
        */
        ...
        if ( is_null($this->packagesResponse) || is_null($this->extrasData) ) {
            ...
            if (sizeof($this->getPackagesList()) == 0) {
                ...
                */
                ...
                $message = 'You are running the dev environment for Updater (no packages are installed).';
                $state = 'dev';
            ...
            if ($this->isCoreUpdateable() == true) {
                ...
                foreach ($this->coreData['coreVersions'] as $i => $v) {
                    if ($i == 0) {
                        ...
                        $updateCandidatesOutput .= "<tr><td>&nbsp;</td></td><td><i class='icon icon-long-arrow-right icon-fw'></i></td><td>${v}</td>";
                    }
                    $tmp = "";
                    ...
                    if ($tmp != '') {
                        $updateCandidatesOutput .= "<td><i class='help icon icon-info-circle' title='${tmp}'></i> </td>";
                    } else {
                        $updateCandidatesOutput .= "<td>&nbsp;</td>";
                    }
                    $updateCandidatesOutput .= "</tr>";
                }
     ...
     * @return string
     */
    private function constructSignature($v = null) {
        if (is_int($v)) {
            return array();
        }
