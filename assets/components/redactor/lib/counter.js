	...
	RedactorPlugins.counter = function()
	{
		return {
			init: function()
			{
                ...
                var messageBox = $(this.$editor).after('<p class="redactor_counter redactor_counter-' + this.uuid + '"><span class="time-wrap"><i class="icon icon-power-off"></i>&nbsp;<span class="reading-time"></span></span>&emsp;&emsp;<span class="word-count-wrap"><i class="icon icon-keyboard-o"></i>&nbsp;<span class="word-count"></span></span></p>');
                ...
                function doKeyUp(that) {
                    ...
                    _rc.find('.reading-time')
                      .text(((_t.hours) ? _t.hours + ':' : '') + ((_t.minutes < 10) ? '0' + _t.minutes.toString() : _t.minutes) + ':' + _t.seconds)
