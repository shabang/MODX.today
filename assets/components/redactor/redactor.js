...
(function($, RedactorPlugins)
{
	...
	Redactor.fn = $.Redactor.prototype = {
		...
		init: function(el, options)
		{
			...
			// load lang #modmore
			this.opts.curLang = $.extend({}, this.opts.langs.en, this.opts.langs[this.opts.lang]);
		...
		inlineMethods: function(type, attr, value)
		{
			...
			{
				this.document.execCommand('fontSize', false, 4 ); // trigger the below loop #modmore
		...
		imageEdit: function(image)
		{
			...
			this.modalInit(this.opts.curLang.edit, this.opts.modal_image_edit, 378, callback);
        ...
        initOverrides: function() {
            this.linkShow = function()
            {
                ...
                var callback = $.proxy(function()
                {
                    ...
                    if (elem && elem.tagName === 'A')
                    {
                        url = elem.getAttribute('href') || elem.href;
                        text = $(elem).text();
                        target = elem.target;
                    ...
                    if(this.opts.advAttrib) $('.redactor_link_adv').show();
                    ...
                    if (this.opts.linkEmail === false && this.opts.linkAnchor === false && this.opts.linkResource === false)
                    {
                        $('#redactor_tabs').remove();
                    ...
                    if($(this.insert_link_node).attr('class')) $('#redactor_link_url_class').val(($(this.insert_link_node).attr('class') || "").replace("redactor-selection-marker",""));
                    if($(this.insert_link_node).attr('id')) $('#redactor_link_url_id').val($(this.insert_link_node).attr('id'));
                    if($(this.insert_link_node).attr('title')) $('#redactor_link_title').val($(this.insert_link_node).attr('title'));
            ...
            this.linkProcess = function()
            {
                ...
                this.linkInsert('<a href="' + link + '"' + target + '>' + text + '</a>', $.trim(text), link, targetBlank, $('#redactor_link_url_class').val(), $('#redactor_link_url_id').val(), $('#redactor_link_title').val());

            };
            
    		this.linkInsert = function (a, text, link, target, linkClass, linkID, linkTitle) {
    			this.selectionRestore();
    			if (text !== '')
    			{
    				if (this.insert_link_node)
    				{
    					...
    					$(this.insert_link_node).attr('class','').text(text).attr('href', link);
                        if(linkClass) $(this.insert_link_node).addClass(linkClass);
                        if(linkID) $(this.insert_link_node).attr('id',linkID);
                        if(linkTitle) $(this.insert_link_node).attr('title',linkTitle);
    		...
    		this.imageEdit = function(image)
    		{
    			...
    			var callback = $.proxy(function()
    			{
    				...
    				$('#redactor_image_edit_src').attr('href', $el.attr('src'));
    				$('#redactor_form_image_align').val($el.css('float'));
                    $('#redactor_link_class').val($el.attr('class'));
                    $('#redactor_link_id').val($el.attr('id'));
                    
                    if(this.opts.advAttrib) $('.redactor_link_adv').show();

    				if ($(parent).get(0).tagName === 'A')
    				{
                        var _ph = $(parent).attr('href');
                        if($(parent).attr('href').substr(0,3) == '[[~') _ph = $(parent).attr('href').replace('[[~','').replace(']]','');
    					$('#redactor_file_link').val(_ph);
			...
			this.imageSave = function(el)
			{
				...
				$el.attr('alt', $('#redactor_file_alt').val());
                if($('#redactor_link_id').val()) $el.attr('id',$('#redactor_link_id').val());

				var floating = $('#redactor_form_image_align').val();


                $el.removeAttr('class');
                if($('#redactor_link_class').val()) $el.addClass($('#redactor_link_class').val());
                if($('#redactor_link_title').val()) $el.attr('title', $('#redactor_link_title').val());
                $el.css({float: '', margin: ''});
	            if (floating === 'left')
	            {
	                $(el).css({ 'float': 'left'});
                        ...
                        $.getJSON(this.opts.fileGetJson, $.proxy(function(data)
                        {
                            ...
                            $.each(data, $.proxy(function(key, val)
                            {
                                ...
                                if(val.message) { /*  !== 'undefined' doesn't work here */
                                	...
                                	$(file).children('a').click(function(){
                            ...
                            if(this.opts.searchImages) {
	                            $('#redactor-image-search').on('input',function(){
	                            	...
	                            	if(_val) {
		                            	...
		                            	$("#redactor_image_box").find(".redactorimage").addClass('hidden').each(function(){
		                            		var _fn = $(this).data('filename').toLowerCase();
		                            		if(_fn.search(_val.toLowerCase()) > -1) $(this).removeClass('hidden');
		                            	}).end();
	                            	} else {
            ...
            $.extend(this.opts, {
               ...
               + '<section>'
                    + '<form id="redactorInsertLinkForm" method="post" action="">'
                        ...
                        + '</div>'
                        + '<div class="redactor_link_adv" style="display:none">'
                        + '<div class="" id="" style="">'
                            + '<label>' + this.opts.curLang.linkTitle + '</label>'
                            + '<input type="text" class="redactor_input redactor_link_title" id="redactor_link_title" />'
                        + '</div>'
                        + '<div class="" id="" style="">'
                            + '<label>' + this.opts.curLang.classLabel + '</label>'
                            + '<input type="text" class="redactor_input redactor_link_class" id="redactor_link_url_class" />'
                        + '</div>'
                        + '<div>'
                            + '<label>' + this.opts.curLang.idLabel + '</label>'
                            + '<input type="text" class="redactor_input" id="redactor_link_url_id" />'
                        + '</div>'
                        + '</div>'
                    + '</form>'
                + '</section>'
				...
				+ '<section id="redactor-modal-image-edit">'
					...
					+ '<input type="text" id="redactor_file_alt" class="redactor_input" />'
					+ '<label>' + this.opts.curLang.link + '</label>'
					+ '<input type="text" id="redactor_file_link" class="redactor_input typeahead" />'
					+ '<label><input type="checkbox" id="redactor_link_blank"> ' + this.opts.curLang.link_new_tab + '</label>'
					+ '<label>' + this.opts.curLang.image_position + '</label>'
					+ '<select id="redactor_form_image_align">'
						+ '<option value="none">' + this.opts.curLang.none + '</option>'
                    ...
                    + '<div>'
					+ '<label>' + this.opts.curLang.idLabel + '</label>'
                    + '<input type="text" id="redactor_link_id" class="redactor_input">'
                    + '</div>'
                    + '</div>'
				+ '</section>'
				+ '<footer>'
    ...
    /**
     * To make the fixed toolbar work nicely within the MODX Manager,
     * we need to change the way the scroll observer works pretty radically.
     *
     * In a nutshell, the offset is relative to the #modx-content (because that
     * is what actually scrolls) and when the boxTop is 0 it means it's right
     * at the top of the screen. We use this and compare it to the offset from
     * the top of the screen to account for the header to figure out when we need
     * to fix the toolbar with position:fixed.
     *
     * We also use this relative offset to figure out when to hide the toolbar,
     * namely when the box is entirely off the screen. This is the end variable.
     */
    $.Redactor.fn.toolbarObserveScroll = function()
    {
        var scrollTop = $(this.opts.toolbarFixedTarget).scrollTop();
        var left = 0;
        var end = 0 - this.$box.height() + 40 + this.opts.toolbarFixedTopOffset;
        var boxOffset = this.$box.offset(),
            boxTop = boxOffset.top,
            boxLeft = boxOffset.left,
            is23 = (MODx.config.connector_url),
            toolbarHeight = this.$toolbar.height(),
            actionBar = (is23) ? $('#modx-action-buttons') : $('#modAB'),
            actionBarNormal = (is23) ? 55 : 85,
            actionBarPushed = actionBarNormal + toolbarHeight;

        // This hack-ish check prevents the toolbar from getting hidden
        //when the editor is hidden by ExtJS (which positions -9999,-9999)
        if (boxLeft < -3000) {
            return;
        }

        if ((boxTop < this.opts.toolbarFixedTopOffset))
        {
            if (boxTop > end) {
                // If the toolbar wasn't fixed before, we move the action bar and position the toolbar fixed
                if (!this.toolbarFixed) {
                    actionBar.css({top: actionBarPushed});
                ...
                // Mark the toolbar as fixed so we can properly handle it later
                this.toolbarFixed = true;
            }
            else {
                if (this.toolbarFixed) {
                    actionBar.css({top: actionBarNormal});
        ...
        {
            // If the toolbar was fixed before, we set the action bar back to normal
            // and also put the toolbar back in place
            if (this.toolbarFixed) {
                actionBar.css({top: actionBarNormal});
                this.$toolbar.css({
                    position: 'relative',
            ...
            // Set a flag so we know the toolbar is currently not fixed
            this.toolbarFixed = false;
